<div class="shadow overflow-hidden sm:rounded-md">
    <div class="px-4 py-5 bg-white sm:p-6">
            ...
            <select id="districtOptions" name="districtId" autocomplete="districtId"
                ...
                @if (!empty($businessData['districts']) && !empty($businessData['provinces']))
                    ...
                    @foreach ($businessData['districts'] as $key => $dist)
                        @foreach ($businessData['provinces'] as $province)
                            @if ($dist->provinceId === $province->id)
                                <option value="{{ $dist->id }}"
                                    {{ $dist->id === $businessData['business']->districtId ? 'selected' : '' }}>
                    ...
                    @foreach ($businessData['municipalities'] as $municipality)
                        @if ($municipality->districtId === $businessData['business']->districtId)
                            <option value="{{ $municipality->id ?? '' }}"
                                @if ($municipality->id === $businessData['business']->municipalityId) selected @endif>
        ...
        <div class="col-span-6 sm:col-span-12">
            ...
            <input type="text" value="{{ $businessData['business']->town ?? '' }}" name="town" id="town"
